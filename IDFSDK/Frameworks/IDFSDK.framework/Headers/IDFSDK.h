...
#import "IDFUserModel.h"

@interface IDFSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(IDFCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(IDFCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，IDFSDKDefines中有声明
 */
- (void)setLanguage:(IDFLanguageType)languageType;
 ...
 *
 */
- (void)login:(IDFCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(IDFCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(IDFCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(IDFCallbackBlock)Block;
 ...
 */
- (void)userInfo:(IDFCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(IDFCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(IDFCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface IDFSDK(IDFIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(IDFCallbackBlock)block ;
